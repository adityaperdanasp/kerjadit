...
**2026-08-25 incident — split @lid/phone-JID identities.** WhatsApp is migrating contacts to a privacy identifier ("@lid") separate from their real `@s.whatsapp.net` phone-number JID; when a contact's account switches to messaging under a fresh @lid, `listener.js` had no way to know it was the same person and just created a second, permanently disconnected `store.json` entry under the new jid. The old (Notion-linked) entry's `lastMessageTimestamp` then sat frozen forever — from the dashboard this looked identical to Case C in the runbook below (days-since-chat frozen), but the actual chat was current, just recorded under an orphaned jid `daily-sync.js` never looks at. Confirmed via `sock.onWhatsApp(...jids)`, which returns `{ jid, exists, lid }` per queried phone JID — the authoritative mapping, not name-matching. Fixed two ways:
- **Ongoing**: `mergeJids()` in `listener.js`, wired into the `contacts.upsert`/`contacts.update` handlers (which already fire live and already carry `c.lid`/`c.jid` — they just weren't being read for this). Folds a newly-seen @lid entry into its existing phone-JID entry the moment WhatsApp reveals the mapping, keeping the phone-JID's `.jid` as the survivor (so `notion-page-map.json`, keyed by `entry.jid` on the daily-sync side, keeps resolving to the existing Notion page instead of creating a duplicate).
- **One-off backfill**: `vps/merge-lid-duplicates.js`. Deliberately does *not* rebuild `store.json` from a fresh history sync like `fetch-history.js` does (that risks silently dropping months of history a resync window wouldn't redeliver) — it loads the *existing* store and actively queries `sock.onWhatsApp()` for every phone-JID contact's `lid`, then applies the same `mergeJids()` merge in place. Must run with `wa-listener.service` stopped (same same-auth-info race as the 2026-08-21 incident). Found 631 split identities out of ~5,355 contacts.

That backfill only fixes `store.json`. Every one of those 631 contacts had *already* been synced to Notion as two separate pages before the fix landed (the orphaned @lid entry looks like a brand-new contact to `daily-sync.js`, so it created one) — 633 duplicate pages total (a couple of `store.json` entries had picked up more than one stray page over time). Cleaned up by cross-referencing `notion-page-map.json` against each merged entry's `allJids`: whichever page id is *not* `pageMap[entry.jid]` (the survivor) is a pre-fix duplicate, safe to archive since its content is already folded into the survivor. Archived (`pages.update({ archived: true })`, i.e. Notion's trash — recoverable, not a hard delete) via a one-off script, not saved in this repo.

If "days-since-chat frozen but they definitely just messaged" comes up again, check for a split identity first: `ssh` in and grep `store.json` for other entries sharing the contact's first name — a `@lid` entry with a recent `lastMessageTimestamp` sitting next to a stale `@s.whatsapp.net` entry for what looks like the same person is this bug, not the Case A/B/C ones below.
